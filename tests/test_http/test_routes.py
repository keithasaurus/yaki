...
class NormalizeRoutesTests(TestCase):
    ...
    def test_two_and_three_tuple_routes(self, test_response_1, test_response_2):
        ...
        normalized_routes = normalize_routes([
            ...
            (bracket_route_matcher("/other/{month}"),
             {"GET", "POST"},
             view_func_1)
        ])
...
class MethodViewToViewFuncTests(TestCase):
    ...
    def test_converts_dicts_to_kwargs(self,
                                      test_request,
                                      test_response,
                                      test_name,
                                      test_age):
        ...
        async def view_request_first(request: HttpRequest,
                                     name: str,
                                     age: str) -> HttpResponse:
    ...
    @given(http_request_named_tuple(),
           http_response_named_tuple())
    def test_dict_method_view_returns_405_for_invalid(self,
                                                      test_request,
                                                      test_response):
        ...
        view_func = method_view_to_view_func(POST,
                                             parsed_params={},
                                             method_view={GET: view,
                                                          PUT: view,
                                                          DELETE: view})

        self.assertEqual(asyncio.run(view_func(test_request)),

...
def regex_match_to_str_dict(
    ...
    if match is None:
        ...
        return {key: str(val) for key, val in match.groupdict().items()}


def regex_route_matcher(pattern: Pattern[str]) -> RouteMatcher:
...
def assert_valid_params(param_names: List[str]) -> None:
    for name in param_names:
        assert name[
            1:-1
        ].isidentifier(), f"{name} is an invalid name for a url parameter"
...
def bracket_route_matcher(url_pattern: str) -> RouteMatcher:
    ...
    processed = [
        (
            to_capturing_bracket_param(s[1:-1])
            if s in set(param_names)
            else re.escape(s)
        )  # need to escape literals
        for s in split_strs
    ]
